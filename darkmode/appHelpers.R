...
myForecast.plot <- function(source, loc, ts.var, ts.end, fore.type, pred.int) {
  ...
  plot(ts.forecast,
     ...
     shadecols = c("#375A7F"),#, "grey70"), #color conf int.
  ...
  lines(data0,type='h',lwd=3.0,col="#DADADA")

  moav <- ma(data0, order=7) 
  lines(moav,col='#8AC3FF',lwd=4) #plot moving average
 ...
 #                 paste(confLevel2, "% Confidence",sep=""))
  ...
  legend("topleft", inset=0.05,
        ...
        col=c("#8AC3FF",'#375A7F'),#,'grey70'),
        lty=c(1,1),lwd=c(4,15),#,15),
...
xy.plot <- function(source, x, y) {
  ...
  (y, string) y variable
  "
  par(bg="#222222", cex=1.2,fg='white',col.axis='white',col.lab='white')
